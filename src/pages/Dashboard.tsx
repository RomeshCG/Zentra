            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm rounded-xl bg-white">
              ...
              <tbody>
                {weeklyCustomers.length === 0 ? (
                  <tr><td colSpan={7} className="text-center text-slate-400 py-4">No customers renewing this week.</td></tr>
                ) : weeklyCustomers.map((c) => (
                  <tr
                    key={c.id}
                    className="border-t hover:bg-cyan-50 transition cursor-pointer"
                    onClick={() => navigate(`/customers?highlight=${c.id}`)}
                  >
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">Rs.{c.income}</td>
                    <td className="px-4 py-2 whitespace-nowrap">Rs.{c.expense}</td>
                    <td className="px-4 py-2 whitespace-nowrap">Rs.{c.profit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <table className="min-w-full border-separate border-spacing-0 text-sm rounded-xl bg-white">
            ...
            <tbody>
              {weeklyManagers.length === 0 ? (
                <tr><td colSpan={4} className="text-center text-slate-400 py-4">No plan managers renewing this week.</td></tr>
